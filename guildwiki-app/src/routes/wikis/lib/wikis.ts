...
import { AREAS, wikisStore } from '$routes/wikis/stores'
...
export type Wikis = {
  publicWikis: WikiInfo[] | null
  privateWikis: WikiInfo[] | null
  selectedArea: AREAS
  loading: boolean
}
...
export const WIKIS_DIRS = {
  [ AREAS.PUBLIC ]: odd.path.directory('public', 'wikis'),
  [ AREAS.PRIVATE ]: odd.path.directory('private', 'wikis')
}
...
export const getImagesFromWNFS: () => Promise<void> = async () => {
  try {
    ...
    wikisStore.update(store => ({ ...store, loading: true }))

    const { selectedArea } = getStore(wikisStore)
    const isPrivate = selectedArea === AREAS.PRIVATE
    const fs = getStore(filesystemStore)

    // Set path to either private or public gallery dir
    const path = WIKIS_DIRS[ selectedArea ]
    ...
    let images = await Promise.all(
      Object.entries(links).map(async ([ name ]) => {
        const file = await fs.get(
          odd.path.combine(WIKIS_DIRS[ selectedArea ], odd.path.file(`${name}`))
        )
...
export const uploadImageToWNFS: (
  ...
  try {
    ...
    const imageExists = await fs.exists(
      odd.path.combine(WIKIS_DIRS[ selectedArea ], odd.path.file(image.name))
    )
    if (imageExists) {
...
export const deleteImageFromWNFS: (
  ...
  try {
    ...
    if (imageExists) {
      ...
      await fs.rm(odd.path.combine(WIKIS_DIRS[ selectedArea ], odd.path.file(name)))
